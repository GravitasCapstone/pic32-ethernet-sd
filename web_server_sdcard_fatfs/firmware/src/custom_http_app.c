...
//bret added this below
#define HTTP_APP_SAVE_FILE
#ifndef NO_MD5
 ...
 ****************************************************************************/
    ...
    #endif
    //bret added this
    #if defined(HTTP_APP_SAVE_FILE)
        static HTTP_IO_RESULT HTTPPostSaveFile(HTTP_CONN_HANDLE connHandle);
    #endif
...
//bret added this
#if defined(HTTP_APP_SAVE_FILE)
    if(!strcmp((char *)filename, "setup.html"))
        return HTTPPostSaveFile(connHandle);
#endif
...
#define HTTP_APP_EMAIL_BODY_SIZE            500 
...
static HTTP_IO_RESULT HTTPPostSaveFile(HTTP_CONN_HANDLE connHandle)
{
    ...
    char dataString[1024];
        ...
        // Try to match the name value
        if(!strcmp(paramName, (const char *)"dataString"))
        {   // Read the dataString name
            ptrParam = dataString;
            paramSize = sizeof(dataString) - 1;
        }
    // Read a form field value
//          Parameters:
//          cData - where to store the value once it is read
//          wLen - how many bytes can be written to cData
        if(TCPIP_HTTP_PostValueRead(connHandle, (uint8_t*)ptrParam, paramSize) == HTTP_READ_INCOMPLETE)
        {
            return HTTP_IO_NEED_DATA;
        }
    //save DataString to csv
    SYS_FS_HANDLE file;
    //open setup.csv
    file = SYS_FS_FileOpen("/mnt/setup.csv",(SYS_FS_FILE_OPEN_WRITE_PLUS));
    //write datastring
    if(SYS_FS_FileWrite(file, (const void *)dataString, strlen(dataString)) != strlen(dataString))
    {
        return HTTP_IO_WAITING;
    }
    //close setup.csv
...
/****************************************************************************
  ...
  Internal:
    See documentation in the TCP/IP Stack APIs or http.h for details.
 ****************************************************************************/
//void TCPIP_HTTP_Print_myName(HTTP_CONN_HANDLE connHandle)
//{
//    const uint8_t *ptr;
//    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
//    ptr = TCPIP_HTTP_ArgGet(TCPIP_HTTP_CurrentConnectionDataBufferGet(connHandle), (const uint8_t *)"brettypoo");
//    if(ptr)
//        TCPIP_TCP_StringPut(sktHTTP, ptr);
//    else
//        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"oof");
//}
...
void TCPIP_HTTP_Print_myName(HTTP_CONN_HANDLE connHandle) {
    ...
    if(TCPIP_TCP_PutIsReady(sktHTTP) < strlen((const char*)"brettypoo"))
    return;
    
    TCPIP_HTTP_CurrentConnectionCallbackPosSet(connHandle, 0x00);
    TCPIP_TCP_StringPut(sktHTTP, (const void*)"brettypoo");

//    TCPIP_HTTP_CurrentConnectionCallbackPosSet(connHandle, 0x00);
//    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "brettypoo");
}
void TCPIP_HTTP_Print_fanstatus(HTTP_CONN_HANDLE connHandle){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"brettypoo");
}
void TCPIP_HTTP_Print_hvacstatus(HTTP_CONN_HANDLE connHandle){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "hot");
}
void TCPIP_HTTP_Print_zoneLoPoint(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    switch (num)
    {
        case 1:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "zone1");
            break;
        case 2:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "zone2");
            break;
        case 3:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "zone3");
            break;
        case 4:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "zone4");
            break;
        default:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "broken, but not really");
    }
}
void TCPIP_HTTP_Print_zoneHiPoint(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
        switch (num)
    {
        case 1:
...
void TCPIP_HTTP_Print_zoneTemp(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
        switch (num)
    {
        case 1:
...
void TCPIP_HTTP_Print_zoneRH(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
        switch (num)
    {
        case 1:
...
void TCPIP_HTTP_Print_zonehvac(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
        switch (num)
    {
        case 1:
...
void TCPIP_HTTP_Print_esuT(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
        switch (num)
    {
        case 1:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "esu1");
            break;
        case 2:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "esu2");
            break;
        default:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "broken, but not really");
    }
}
void TCPIP_HTTP_Print_esuRH(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    switch (num)
        {
        case 1:
...
void TCPIP_HTTP_Print_esuP(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
        switch (num)
        {
        case 1:
...
void TCPIP_HTTP_Print_splitT(HTTP_CONN_HANDLE connHandle){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "Split Temperature");
}
void TCPIP_HTTP_Print_splitRH(HTTP_CONN_HANDLE connHandle){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "Split RH%");
}
void TCPIP_HTTP_Print_splitP(HTTP_CONN_HANDLE connHandle){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "Split Pressure");
}
void TCPIP_HTTP_Print_damperZ(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
            switch (num)
    {
        case 1:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper1");
            break;
        case 2:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper2");
            break;
        case 3:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper3");
            break;
        case 4:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper4");
            break;
        case 5:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper5");
            break;
        case 6:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper6");
            break;
        case 7:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper7");
            break;
        case 8:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "damper8");
            break;  
        default:
            TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) "broken, but not really");
    }
}
void TCPIP_HTTP_Print_damperOp(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    switch (num)
    {
        case 1:
...
void TCPIP_HTTP_Print_damperOpt(HTTP_CONN_HANDLE connHandle,uint16_t num){
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    switch (num)
        {
        case 1:
